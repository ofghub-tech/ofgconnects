...
export default function BiblePanel() {
    ...
    // --- MODIFIED: Buttons are now semi-transparent ---
            ...
            <div className="grid grid-cols-2 gap-2 md:grid-cols-3">
                {filteredBooks.map(book => (
                    <button
                        ...
                        className="w-full p-3 text-left bg-white/50 border border-gray-200/50 rounded-lg dark:bg-gray-700/50 dark:border-gray-600/50 hover:bg-white/80 dark:hover:bg-gray-600/80 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
            ...
            <div className="grid grid-cols-5 gap-2 md:grid-cols-6 lg:grid-cols-7">
                {chapters.map(chapterNum => (
                    <button
                        ...
                        className="flex items-center justify-center w-full text-base font-medium text-gray-700 bg-gray-100/50 border border-gray-300/50 rounded-lg shadow-sm dark:bg-gray-700/50 dark:border-gray-600/50 dark:text-gray-200 aspect-square hover:bg-blue-100/50 dark:hover:bg-blue-800/50 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                        {chapterNum}
                    </button>
                ))}
        ...
        <div className="flex-1 p-4 overflow-y-auto">
            {loadingVerses ? <p className="text-gray-500 dark:text-gray-400">Loading...</p> : (
                <div className="text-gray-800 dark:text-gray-100">
                    {verses.map(verse => (
                        <div key={verse.$id} className="py-3 border-b border-gray-200/50 dark:border-gray-700/50 last:border-b-0">
    ...
    return (
        // --- MODIFIED: Applied .glass-panel class and removed redundant styles ---
        <div className="glass-panel flex flex-col w-full h-full overflow-hidden p-0 text-gray-900 dark:text-gray-100">
            {/* --- MODIFIED: Header is now semi-transparent --- */}
            <header className="flex items-center justify-between p-3 border-b border-white/20 dark:border-gray-700/50 flex-shrink-0 bg-transparent">
                <div className="w-10">
                    {(selectedBook || selectedChapter) && (
                        <button onClick={handleBack} className="p-1.5 text-gray-600 rounded-full dark:text-gray-300 hover:bg-gray-200/50 dark:hover:bg-gray-700/50" aria-label="Go back">
                            <ChevronLeftIcon />
                        </button>
                    )}
                </div>
                ...
                <div className="w-10 flex justify-end">
                    {bibleView === 'sidebar' ? (
                        <button onClick={openBibleFullscreen} className="p-1.5 text-gray-600 rounded-full dark:text-gray-300 hover:bg-gray-200/50 dark:hover:bg-gray-700/50" aria-label="Maximize">
                            <MaximizeIcon />
                        </button>
                    ) : (
                        <button onClick={openBibleSidebar} className="p-1.5 text-gray-600 rounded-full dark:text-gray-300 hover:bg-gray-200/50 dark:hover:bg-gray-700/50" aria-label="Minimize">
                            <MinimizeIcon />
                        </button>
                    )}
                </div>
