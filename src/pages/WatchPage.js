...
const WatchPage = () => {
    ...
    // --- FIX: STRICT UNIQUE VIEW LOGIC ---
    const logVideoView = async (userId, videoId, currentViewCount) => {
        if (!userId) return null; 

        try {
            // 1. Check if ANY history exists for this user+video combination
            ...
            // If they have watched it before, do not count again.
            if (historyCheck.total > 0) {
                return null;
            }

            // 2. Unique viewer: Log history
    ...
    if (loading) return <div className="flex w-full h-[70vh] items-center justify-center bg-white dark:bg-gray-900"><p className="text-neutral-500 dark:text-gray-400">Loading...</p></div>;
    if (!video) return <div className="flex w-full h-[70vh] items-center justify-center bg-white dark:bg-gray-900"><p className="text-red-600">Video not found.</p></div>;
    ...
    return (
        <div className="w-full bg-white text-neutral-900 p-4 sm:p-6 lg:p-8 dark:bg-gray-900 dark:text-gray-100">
            <div className="max-w-screen-xl mx-auto grid grid-cols-1 lg:grid-cols-3 lg:gap-x-6 gap-y-6">
                    ...
                    {video.username && (
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 py-3">
                            ...
                            <div className="flex items-center gap-2 w-full sm:w-auto">
                                ...
                                <button
                                    className={`flex items-center justify-center gap-2 py-2 px-4 h-9 rounded-full font-medium text-sm bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-100 dark:hover:bg-gray-600 transition-colors disabled:opacity-50 ${isSaved ? 'bg-gray-200 font-semibold dark:bg-gray-600' : ''}`}
                ...
                <div className="lg:col-span-1"><SuggestedVideos currentVideo={video} /></div>
            </div>
        </div>
    );
};
