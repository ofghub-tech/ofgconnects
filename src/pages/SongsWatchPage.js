...
// This file is 99% identical to WatchPage.js
...
import SuggestedVideos from '../components/SuggestedVideos'; // We re-use the same component
...
const SongsWatchPage = () => { // <-- Renamed component
    ...
    // --- (Watch Later functions: checkSavedStatus, toggleSavedStatus - No change) ---
    ...
    // --- (History logging: logVideoView - No change) ---
    ...
    // --- (useEffect hook to fetch video - No change) ---
    useEffect(() => {
    ...
    if (loading) {
        return (
            ...
            <div className="flex w-full h-full min-h-[70vh] items-center justify-center p-10 bg-white dark:bg-gray-900">
                <p className="text-xl text-neutral-500 dark:text-gray-400">Loading song...</p>
            </div>
        );
    }
    if (!video) {
        return (
            // --- MODIFIED: Added dark mode classes ---
            <div className="flex w-full h-full min-h-[70vh] items-center justify-center p-10 bg-white dark:bg-gray-900">
                <p className="text-xl text-red-600">Song not found or failed to load.</p>
            </div>
        );
    }
    // --- (End States) ---
...
export default SongsWatchPage; // <-- Export new component